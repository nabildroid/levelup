...
import {
    ...
    PubsubSources,
    PubsubInsertTaskAttributes,
    PubsubValidateTaskAttributes,
    PubsubDetectedEventTypeMessageType,
} from "../types/pubsub";
...
export default class PubSubConnector {
    ...
    constructor(client: PubSub) {
        ...
        if (isDev()) {
            Object.values(PubSubConnector.pubsubTopics).forEach(
                async (name) => {
    ...
    insertTask(task: Task, source: PubsubSources) {
        ...
        return this.client
            .topic(PubSubConnector.pubsubTopics.INSERT_TASK)
    ...
    validateTask(taskId: NTID, source: PubsubSources) {
        ...
        return this.client
            .topic(PubSubConnector.pubsubTopics.VALIDATE_TASK)
            .publishJSON({ id: taskId }, attribute);
    }

    detectedEventType<T extends PubsubDetectedEventTypeAttributes>(
        data: PubsubDetectedEventTypeMessageType[T["type"]],
        attribute: T
    ) {
        this.log(attribute.type);
        return this.client
            .topic(PubSubConnector.pubsubTopics.DETECTED_TASK_EVENT)
