...
import {
    NotionDb,
    NotionDbType,
    NotionServerTaskDBReponse,
    NotionTask,
    NotionTaskCreate,
    NotionServerSingleTaskResponse,
    NotionTaskUpdate,
} from "../types/notion";
...
export default class Notion{
    ...
    protected async checkForNewTasks(db: NotionDb): Promise<NotionTask[]> {
        ...
        return database.results
            .map((page) =>
                Notion.convertSingleTaskPageResponseToNotionTask(page, db.id)
            )
            .filter(
                ({ last_edited }) =>
                    !last_edited || last_edited >= db.lastRecentDate.toDate()
            );
    }

    // todo refactor this please :( !
    static convertSingleTaskPageResponseToNotionTask(
        task: NotionServerSingleTaskResponse,
        parent: string
    ): NotionTask {
        return {
            id: task.id,
            parent,
            title: task.properties.title.title
                .map((t) => t.plain_text)
                .join(" "),
            done: task.properties.done.checkbox,
            labels: task.properties.labels.multi_select.map(
                (s) => s.name as string
            ),
            priority: task.properties.priority?.select.name
                ? toPriority(task.properties.priority.select.name)
                : undefined,
    ...
    static convertNotionTaskToSingleTaskPageProperties(
        ...
        taskPropertyNames.forEach((key) => {
            if (key == "title")
                ...
                properties[key] = {
                    type: "multi_select",
                    multi_select: (task.labels as string[]).map((l) => ({
                        name: l,
                    })),
                };
